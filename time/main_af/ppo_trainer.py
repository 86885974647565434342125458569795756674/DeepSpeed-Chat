...
class DeepSpeedPPOTrainer():
    ...
    def __init__(self, rlhf_engine, args):
        ...
        # In case the generatea experience is not valid (too short), we use the last valid
...
#cyy
            for _ in range(3):
                seq1=seq.clone()
                seq=torch.cat((seq,seq1),0)
                attention_mask1=attention_mask.clone()
                attention_mask=torch.cat((attention_mask,attention_mask1),0)
            seq1=seq[:8].clone()
            seq=torch.cat((seq,seq1),0)
            attention_mask1=attention_mask[:8].clone()
            attention_mask=torch.cat((attention_mask,attention_mask1),0)
            ...
            if self.args.local_rank==0:
                ...
                print("bs={},cyy_af".format(len(seq)), start.elapsed_time(end))
            else:
                output = self.actor_model(seq, attention_mask=attention_mask)
  ...
  #      logits_ref = output_ref.logits
        logits_ref=logits
        values=torch.randn(logits.shape[0],logits.shape[1]-1,device=logits.device)
        reward_score=torch.randn(logits.shape[0],device=logits.device)
