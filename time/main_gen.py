...
def parse_args():
    ...
    parser.add_argument("--tensorboard_web",type=str)
    parser.add_argument(
        '--data_path',
        nargs='*',
...
def main():
    ...
    for epoch in range(args.num_train_epochs):
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            #cyy
            if step<2:
                _=trainer._generate_sequence(batch_prompt['prompt'], batch_prompt['prompt_att_mask'], step)
                continue
            #cyy
            if args.local_rank==0:
                ...
                start.record()
                _=trainer.generate_experience(batch_prompt['prompt'], batch_prompt['prompt_att_mask'], step)
                end.record()
                torch.cuda.synchronize() 
                print("step{},bs={}".format(step,len(batch_prompt['prompt'])),"cyy_gen", start.elapsed_time(end))
                if step==4:
                    import sys
                    sys.exit()
            else:
